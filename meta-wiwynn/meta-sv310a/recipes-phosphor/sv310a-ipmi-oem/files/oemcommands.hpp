...
static constexpr ipmi_netfn_t netFnSv310aOEM1 = 0x3A;
static constexpr ipmi_netfn_t netFnSv310aOEM2 = 0x30;
static constexpr ipmi_netfn_t netFnSv310aOEM3 = 0x3E;
static constexpr ipmi_netfn_t netFnSv310aOEM4 = 0x3C;
static constexpr ipmi_netfn_t netFnSv310aOEM5 = 0x34;
static constexpr ipmi_netfn_t netFnSv310aOEM6 = 0x36;

static constexpr int gpioJtagBmcOeN = 2;
static constexpr int gpioBmcJtagSel = 148;

static const size_t maxPatternLen = 256;
static const size_t maxfwVersionLen = 14;
static const size_t retPostCodeLen = 20;
static const uint8_t oemMaxIPMIWriteReadSize = 50;

enum ipmi_sv310a_oem1_cmds : uint8_t
{
    CMD_RANDOM_POWERON_STATUS = 0x15,
    CMD_RANDOM_POWERON_SWITCH = 0x16,
};

enum ipmi_sv310a_oem2_cmds : uint8_t
{
    ...
    CMD_GET_AMBER_LED_STATUS = 0x20,
    CMD_GET_POST_END_STATUS = 0xAA,
};

enum ipmi_sv310a_oem3_cmds : uint8_t
{
...
enum ipmi_sv310a_oem4_cmds : uint8_t
{
    CMD_GET_VR_VERSION = 0x51,
    CMD_GET_FW_VERSION = 0x80,
    CMD_ACCESS_CPLD_JTAG = 0x88,
};

enum ipmi_sv310a_oem5_cmds : uint8_t
{
    ...
    CMD_SET_CPU_POWER_LIMIT = 0x06,
};

enum ipmi_sv310a_oem6_cmds : uint8_t
{
...
enum vr_addr : uint8_t
{
    VR_ADDR_CPU_CORE = 0x61,
    VR_ADDR_CPU_SOC = 0x63,
    VR_ADDR_MEM_ABCD = 0x64,
    VR_ADDR_MEM_EFGH = 0x65,
};
...
constexpr auto manualModeFilePath = "/var/fanManual/";
...
typedef struct
{
    uint8_t component;
    uint8_t index;
}__attribute__((packed)) GetFwVersionReq;

typedef struct
{
    uint8_t fwVersion[maxfwVersionLen];
}__attribute__((packed)) GetFwVersionRes;
...
} __attribute__((packed)) SetCpuPowerLimitCmdReq;