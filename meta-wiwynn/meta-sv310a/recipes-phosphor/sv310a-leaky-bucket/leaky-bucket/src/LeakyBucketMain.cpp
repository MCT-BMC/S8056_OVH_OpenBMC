...
bool updateThresholdJson(std::string thresholdName, uint16_t Value)
{
    ...
    // Check JSON syntax
    if (thresholdData.is_null() || thresholdData.is_discarded())
    {
        if (!createDefaultThresholdFile())
        {
            return false;
...
bool setThreshold1(const uint16_t& newValue, uint16_t& oldValue)
{
...
bool setThreshold2(const uint16_t& newValue, uint16_t& oldValue)
{
...
bool setThreshold3(const uint16_t& newValue, uint16_t& oldValue)
{
...
bool createThresholdProperties(sdbusplus::asio::object_server& objectServer,
                               std::shared_ptr<sdbusplus::asio::dbus_interface>&
                                   thresholdPropertyInterface)
{
    // 1. Check if the threshold config file exists.
    struct stat buf;
    if (0 != stat(thresholdConfigPath.c_str(), &buf))
    {
        // If the threshold config file does not exit, create a new one with
        // default values.
...
bool createDimmEccObjects(
    ...
    boost::container::flat_map<std::string, std::unique_ptr<LeakyBucketObject>>&
        dimmobjs)
{
...
int main(int argc, char* argv[])
{
    ...
    io.post([&]() { createDimmEccObjects(io, objectServer, dimmobjs); });
