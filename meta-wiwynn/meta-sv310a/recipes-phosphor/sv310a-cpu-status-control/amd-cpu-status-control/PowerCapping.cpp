...
PowerCapping::PowerCapping(
    ...
    defaultCpuPowerLimit(DEFAULT_POWER_LIMIT)
{
    // Initialize psuQueue
    ...
    if (pwrCapStatusFile.is_open())
    {
        ...
        {
            autoPowerCap = data.value("autoPowerCap", "OFF");
            systemPowerLimit = data.value("systemPowerLimit", 0);
            manualPowerCap = data.value("manualPowerCap", "OFF");
            cpuPowerLimit = data.value("cpuPowerLimit", 0);
            if (autoPowerCap == "ON")
            {
                isAsserted = true;
            }
    ...
    methodInterface->register_method(
        "SetCPUPowerLimit", [this](int sockId, uint32_t power) {
            manualPowerCap = "ON";
            operationalInterface->set_property("manualPowerCap",
                                               manualPowerCap);
            cpuPowerLimit = power;
            return setCpuPowerLimit(sockId, cpuPowerLimit);
        });
    ...
    if (!operationalInterface)
    {
        ...
        operationalInterface->register_property("autoPowerCap", autoPowerCap);

        operationalInterface->register_property("manualPowerCap",
                                                manualPowerCap);
    ...
    if (!parameterInterface)
    {
        ...
        parameterInterface->register_property(
            "DefaultCpuPowerLimit", defaultCpuPowerLimit,
            [this](const uint32_t propIn, uint32_t& old) {
                if (propIn == old)
                {
                    return 1;
                }
                old = propIn;
                defaultCpuPowerLimit = propIn;
                return 1;
            });
    ...
    auto powerStatusMatcherCallback = [this](sdbusplus::message::message& msg) {
        ...
        if (findPlatformRstStatus != propertiesChanged.end())
        {
            ...
            if ((*platfromRstStatus == true) && (autoPowerCap == "ON"))
            {
                waitTimer.cancel();
                if (operationalInterface)
                {
                    autoPowerCap = "PAUSE";
                    operationalInterface->set_property("autoPowerCap",
                                                       autoPowerCap);
                }
                if (debugModeEnabled)
                {
                    std::cerr << "[Debug Mode]"
                              << "Auto mode - pause to cap CPU power\n";
                }
            }
            if ((*platfromRstStatus == true) && (manualPowerCap == "ON"))
            {
                if (operationalInterface)
                {
                    manualPowerCap = "PAUSE";
                    operationalInterface->set_property("manualPowerCap",
                                                       manualPowerCap);
                }
                if (debugModeEnabled)
                {
                    std::cerr << "[Debug Mode]"
                              << "Manual mode - pause to cap CPU power\n";
                }
            }
        }
        else if (findPostCompleteStatus != propertiesChanged.end())
        {
            ...
            if ((*postCompleteStatus == 0) && (!isCrashdump()))
            {
                if (autoPowerCap == "PAUSE")
                {
                    ...
                    {
                        std::cerr << "[Debug Mode]"
                                  << "Auto mode - restart to cap CPU power\n";
                    }
                }
                if (manualPowerCap == "PAUSE")
                {
                    ...
                    {
                        std::cerr << "[Debug Mode]"
                                  << "Manual mode - restart to cap CPU power\n";
                    }
                }
            }
        }
    ...
    /* TODO: Register a matcher to listen the crashdump state.
       ...
       Discard this event if the autoPowerCap is "OFF".
       When crashdump state is true and the autoPowerCap is "ON"
         (1) Stop timer to stop capping power.
         (2) Change parameter "autoPowerCap" to "PAUSE" and set property "autoPowerCap" on dbus.
       ...
       When crashdump state is false, host is power-on and BIOS post complete
         ...
         (2) Change parameter "autoPowerCap" to "ON" and set property "autoPowerCap" on dbus.
         (3) Print debug message if the parameter "debugModeEnabled" is true.
     */
    ...
    if (autoPowerCap == "ON")
    {
        ...
        while (retryStartPwrCapTime <= maxStartPwrCapRetryTime)
        {
            ...
            {
                fprintf(stderr,
                        "[%s] Auto mode - failed to start to cap power\n",
                        __func__);
                retryStartPwrCapTime = 0;
                break;
            }
    ...
    else if (manualPowerCap == "ON")
    {
        ret = setCpuPowerLimit(cpuSocketId, cpuPowerLimit);
        while (retryStartPwrCapTime <= maxStartPwrCapRetryTime)
        {
            ret = setCpuPowerLimit(cpuSocketId, cpuPowerLimit);
            if (ret == 0)
            ...
            {
                fprintf(stderr,
                        "[%s] Manaul mode - failed to start to cap power\n",
                        __func__);
                retryStartPwrCapTime = 0;
                break;
            }
...
int PowerCapping::readPsuPin()
{
    ...
    auto method =
        bus.new_method_call("xyz.openbmc_project.PSUSensor",
                            "/xyz/openbmc_project/sensors/power/HSC_Input_Power",
...
int PowerCapping::setParameters(int sockId, uint32_t power)
{
    ...
    {
        autoPowerCap = "ON";
        operationalInterface->set_property("autoPowerCap", autoPowerCap);
        manualPowerCap = "OFF";
        operationalInterface->set_property("manualPowerCap", manualPowerCap);
    }
    ...
    if (!isAsserted)
    {
        ...
        if (ipmiSelAdd(message,
                       "/xyz/openbmc_project/sensors/event/Server_Power_Cap",
                       eventData, true) != 0)
        {
...
{
    oob_status_t ret;
    uint32_t max_power = MAX_POWER_LIMIT;
...
int PowerCapping::setDefaultCpuPowerLimit(int sockId)
{
    ...
    ret = write_socket_power_limit(sockId, defaultCpuPowerLimit);
    if (ret != OOB_SUCCESS)
    {
        fprintf(stderr,
